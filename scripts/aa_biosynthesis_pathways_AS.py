...
def main(biosyn_pathways_fname, all_genes_fname, aa_dist_by_gene_fname, proteomics_fname, output_fname):

    AA_LETTERS = sorted("ACEDGFIHKMLNQPSRTWVY")
    AA_PATH = sorted(['P_'+ x for x in AA_LETTERS])
    
    aa_biosythesis_reader = csv.reader(open(biosyn_pathways_fname), delimiter='\t')
    genes_in_path = {}
    for i, row in enumerate(aa_biosythesis_reader):
         bnumbers= filter(None, row[1:])
         UPIDs = ds.convert_bnumbers_to_UPID(bnumbers, all_genes_fname)
         genes_in_path[AA_LETTERS[i]] = UPIDs
    
    UPID_to_aa_dist = ds.load_UPID_to_aa_dist(aa_dist_by_gene_fname)
    aa_dist_in_path = {}
...
if __name__ == "__main__":
    main('aa_biosynthesis_pathways_genes.csv', 'all_ecoli_genes.txt', 'aa_dist_by_UP_ID.csv', 
         'Ecoli_19_Conditions_Proteomics.csv', 'aa_biosynthesis_pathways_vs_proteome_aa_dist.pdf')
